...
bot.command("cekfunc", async (ctx) => {
  try {
    ...
    if (!reply || !reply.document)
      return ctx.reply("❌ Reply file *.js lalu ketik /cekfunc")
...
{ parse_mode: "Markdown" }
      ...
      await ctx.reply(
`\`\`\`
✅ Yeay Function Aman & tidak ditemukan error syntax.
\`\`\``,
{ parse_mode: "Markdown" }
      )
      ...
      // INFO ERROR (TANPA CODE BLOCK)
      await ctx.reply(
`❌ ERROR TERDETEKSI!
${err.message}
Lokasi: Line ${line}:${col}`
      )

      // CODE BLOCK KHUSUS CUPLIKAN
      await ctx.reply(
`\`\`\`js
${snippet}
\`\`\``,
